...
- **Avtorizatsiya yordamchi funksiyalari — yagona haqiqat manbai:** yuqoridagi "admin ko'radi" va "tegishli murojaat/nizo egasi ko'radi" mantig'i har bir siyosatda alohida-alohida takrorlanmaydi — to'rtta qayta ishlatiladigan SQL funksiya orqali markazlashtirilgan: `public.is_admin()`, `public.owns_appeal(uuid)`, `public.is_dispute_party(uuid)` va `public.can_access_case(case_type, uuid, uuid)`. Bu funksiyalar faqat boolean qaytaradi, hech qanday qator ma'lumotini oshkor qilmaydi va rekursiya xavfsizligi uchun qat'iy qoidaga bo'ysunadi: `is_admin()` `profiles` jadvalining o'z siyosatida ham ishlatilgani sababli `SECURITY DEFINER` (RLS'ni chetlab o'tadi, shu bilan o'z-o'ziga bog'liq rekursiyani tuzilmaviy ravishda yo'q qiladi); `owns_appeal()`/`is_dispute_party()` esa faqat **boshqa** jadvallar siyosatida ishlatiladi — `appeals`/`disputes`ning o'z siyosatlarida hech qachon ishlatilmaydi (aks holda o'z-o'ziga bog'liq rekursiya xavfi tug'iladi), shu sababli `SECURITY INVOKER` yetarli. `storage.objects` siyosatlarida `can_access_case()` ishlatilmaydi — u yerdagi `case_type` fayl yo'lidan olingan xom text segment bo'lgani uchun, buni enumga cast qilish keraksiz xavf qo'shadi; o'rniga xavfsiz UUID regex tekshiruvidan keyin `owns_appeal()`/`is_dispute_party()` to'g'ridan-to'g'ri chaqiriladi. To'liq implementatsiya: `supabase/migrations/20260728000001_authorization_hardening.sql`.
