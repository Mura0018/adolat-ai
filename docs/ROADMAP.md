...
- **Bosqich 6 — Mustahkamlash, audit va reliz tayyorgarligi:** Security/Performance/UX auditlari, aniqlangan kamchiliklarni yopish (`docs/ACTION_PLAN.md` orqali), reliz oldidan yakuniy tekshiruv (`docs/SECURITY.md`, "Security Checklist" bo'limi). **Old shart (ikkita mustaqil shart, ikkalasi ham qondirilishi kerak):** (1) bu bosqich Bosqich 1, 3, 4 va 5 funksional jihatdan yakunlangandan keyingina boshlanadi — u "oxirgi jilolash" bosqichi, "keyingi ish bloki" degani emas; (2) `docs/adr/README.md`da belgilangan ADR gate — kamida ADR-001 va ADR-002 (Critical darajadagi Architecture Decision Record'lar) **Qabul qilingan** holatida bo'lishi kerak. Bu ikkinchi shart birinchisidan mustaqil — Bosqichlar funksional yakunlangan bo'lsa ham, ADR gate qondirilmagan bo'lishi mumkin (yoki aksincha).
